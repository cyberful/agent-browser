...
Use `agent-browser skills list` to see all available skills, then `agent-browser skills get <name>` to load one. `agent-browser skills get core --full` is the recommended starting point for CLI-driven browser tasks. Managed MCP clients whose host owns runtime configuration and process lifetime should load `core-mcp-managed` instead.
