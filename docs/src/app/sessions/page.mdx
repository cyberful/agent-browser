...
The Cyberful hardened build can open a persistent profile for direct human login without attaching DevTools to any page target:
...
The daemon owns the Chrome process so `close` and process-tree cleanup still work, but it does not create, navigate, reset, inspect, or subscribe to tabs. Complete login in the visible browser, close the passive session, then relaunch without `AGENT_BROWSER_PASSIVE` to automate the authenticated profile. Passive mode accepts only local headed Chrome and rejects remote CDP, providers, authenticated proxies, and allowed-domain interception.
